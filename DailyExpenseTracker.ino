...
void loop() {
  ...
  switch (steps) {  // ini biar ngeloop tiap step soalnya aku pake button, SWITCH GAK NGELOOP SENDIRI ini cuman karena di dalem loopnya ESP32 makanya bisa nge loop
    case 0:         // MENU
      {
        String MENU[] = { "Tracker", "Upload", "Settings", "Storage" };
        int MENUMap[] = { 1, 5, 6, 7 };
    ...
    case 1:
      {  // step 1 cari masukan dari pengguna
        ...
        if (reset == 0 && lastreset == 1) {
          ...
          if (save > 0) {  // check kalo penghasilannya lebih dari 0,
            ...
            OLEDprint("twin... penghasilan mu gak boleh 0", "", "");
            delay(5000);
            steps = 0;
    ...
    case 2:
      {  // step 2 cari pengeluaran dari pengguna
        if (UP == 0 && lastUP == 1) {
          count++;
          OLEDprint("pengeluaran = ", String(count));
          delay(100);
    ...
    case 3:
      {  // step 3 total nabungnya brp dan skor
    ...
    case 7:
      {
        ...
        OLEDprint("Total: " + String(totalBytes), "Used : " + String(usedBytes), "Free : " + String(totalBytes - usedBytes));
    ...
    default:
      {
        OLEDprint("not implemented ", "returning to menu");
        delay(5000);
        steps = 0;
        IsMenuDisplayed = false;
        break;
...
void InitOLED() {
  ...
  display.setCursor(0, 0);
  display.println("Initilizing OLED");
  display.display();
  delay(1000);
}
void initSPIFFS() {
  if (!SPIFFS.begin(false)) {
    OLEDprint("SPIFFS Mount Failed");
    display.display();
    delay(1000);  // Let user see the message

    OLEDprint("Erasing SPIFFS...");
    display.display();
    delay(1000);

    if (SPIFFS.format()) {
      OLEDprint("SPIFFS erased");
    } else {
      OLEDprint("Erase error!");
    }
    display.display();
    delay(1000);
  } else {
    OLEDprint("SPIFFS ready!");
    display.display();
    delay(500);
  }
}
